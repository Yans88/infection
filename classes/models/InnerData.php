...
class InnerData
{
  /*
    下記のJSONを表すモデルです
    It is a model that represents the following JSON
  ...
  */
   ...
   * Inner Items
   *
   * @var Array $inner_item
   */
  public $inner_item;

  public function __construct(array $common, array $absence, array $class_def, array $disorder_head, array $disorder_ditale, array $close_data, array $inst, array $inner_item)
  {
    $this->filecode = '';

    foreach ($common as $val) {
      if (!$val instanceof InnerData_Common) {
    ...
    foreach ($absence as $val) {
      if (!$val instanceof InnerData_Absence) {
        throw new Exception('Invalid Data');
      }
    }
    ...
    foreach ($class_def as $val) {
      if (!$val instanceof InnerData_ClassDef) {
        throw new Exception('Invalid Data');
      }
    }
    ...
    foreach ($disorder_head as $val) {
      if (!$val instanceof InnerData_DisorderHead) {
        throw new Exception('Invalid Data');
      }
    }
    ...
    foreach ($disorder_ditale as $val) {
      if (!$val instanceof InnerData_DisorderDitale) {
        throw new Exception('Invalid Data');
      }
    }
    ...
    foreach ($close_data as $val) {
      if (!$val instanceof InnerData_CloseData) {
        throw new Exception('Invalid Data');
      }
    }
    ...
    foreach ($inst as $val) {
      if (!$val instanceof InnerData_Inst) {
        throw new Exception('Invalid Data');
      }
    }
    $this->inst = $inst;

    foreach ($inner_item as $val) {
      if (!$val instanceof InnerData_InnerItem) {
        throw new Exception('Invalid Data');
      }
    }
...
class InnerData_Absence
{
  ...
  {
    return intval($this->absence_num);
  }
}
...
class InnerData_DisorderDitale
{
  /**
   * Data Classification
   *
...
class InnerData_InnerItem
{
  ...
  public function setClassNumAdd($class_num_add)
  {
    $this->class_num_add = $class_num_add;
  }

  public function getClassNumAdd()
  {
    return $this->class_num_add;
  }
}
